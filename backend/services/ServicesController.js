class Services {
    ...
    async atualizar(req, res, next) {
        try {
            const registroAtualizado = await this.model.findByIdAndUpdate(req.params.id, req.body, { new: true });
            if (!registroAtualizado) return res.status(404).json({ message: "Registro não encontrado" });
    ...
    async deletar(req, res, next) {
        try {
            ...
            if (!registroDeletado) return res.status(404).json({ message: "Registro não encontrado" });
